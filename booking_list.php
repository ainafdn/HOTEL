...
WHERE idcat='$Room[idcat]'");
$Cat=mysqli_fetch_array($dataCat);


//LINK TO TABLE GUEST VIA FOREIGN KEY 
$dataGuest=mysqli_query($samb,"SELECT * FROM guest
WHERE icguest='$Booking[icguest]'");
...
$Out = date("d-m-Y", strtotime($Booking['check_out']));
   ...
   <tr>
	...
	<td>RM <?php echo number_format($Cat['price']*$totalday,2);
	$totalAmount+=($Cat['price']*$totalday);?></td>
